window.onload = () => {
    ...
    class Tetromino {
        ...
        constructor(xs, ys, color = null) {
            ...
            if (color !== null) {
                this.color = color;
                this.img = new Image();
                this.img.src = `../images/resources/${Tetromino.COLORS[color]}.jpg`;
            }
        }
    ...
    let tetromino = null,
        ...
        score,
        lines,
        gameInterval = null; // Variable para controlar el loop

    // === Inicialización del canvas y el campo ===
    (function setup() {
        ...
        reset(); // Inicializa score, lines y campo
    })();
    ...
    function draw() {
            ...
            if (tetromino.collides(i => ({ x: tetromino.x[i], y: tetromino.y[i] + 1 }))) {
                ...
                for (let y = FIELD_HEIGHT - 1; y >= MIN_VALID_ROW; --y) {
                    if (FIELD[y].every(e => e !== false)) {
                        for (let ay = y; ay >= MIN_VALID_ROW; --ay) FIELD[ay] = [...FIELD[ay - 1]];
                        completedRows++;
                        y++;
                    }
                }
        ...
        gameInterval = setTimeout(draw, delay);
    }
